<x-app-layout>
        ...
        <div class="w-full max-w-lg">
            <div class="flex items-center border-b border-b-2 border-blue-500 py-2">
                <input id="search-input"
                    ...
                    type="text" placeholder="Search for schools" aria-label="Search">
            </div>
        </div>
    </div>
    ...
    <div class="r mt-8">
        <div class="overflow-x-auto">
            <table id="schools-table" class="table-auto min-w-full divide-y divide-gray-200">
                <thead class="bg-blue-600">
                    <tr>
                        <th class="px-6 py-5 text-left text-xs font-bold text-white uppercase tracking-wider">#</th>
                        <th class="px-6 py-5 text-left text-xs font-bold text-white uppercase tracking-wider">School
                            Name</th>
                ...
                <tbody>
                    ...
                    <tr class="{{ $count % 2 == 0 ? 'bg-blue-200' : '' }}">
                        ...
                        <td class="px-6 py-6 whitespace-nowrap">{{ $school->name }}</td>
    ...
    <script>
        document.getElementById('search-input').addEventListener('input', function() {
            ...
            let rows = document.querySelectorAll('#schools-table tbody tr');

            rows.forEach(row => {
                let schoolName = row.querySelector('td:nth-child(2)').textContent.toLowerCase();
